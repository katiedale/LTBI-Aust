...
yoaA<-merge(Aust, tbhazl, by.x = c("yoa"), by.y = c("year"), all.x=TRUE, all.y=FALSE)
yoaA[ , (8:207) :=lapply(.SD, "*", 0.25), .SDcols =c(8:207)]
...
subyob <- merge(subyoaA[,c(1:7,208)], subyob,by=c('id2'))
subyob[, "id2":=NULL]
...
yoaA[ , (8:207) :=lapply(.SD, "*", 0.5), .SDcols =c(8:207)]
yoaA<-yoaA[order(iso3,yob,yoa)]
...
yoa <- merge(Aust[,1:7], yoa,by=c('id'))
## remove the hazard values in the rows where yob equals yoa, 
#because I sorted their hazard values above
...
yoacmsm<-merge(Aust, tbhazl, by.x = c("yoa"), by.y = c("year"), all.x=TRUE, all.y=FALSE)
yoacmsm<-yoacmsm[order(iso3,yob,yoa)]
# creating data table with cumulative hazard in the censusyear
Aust<-Aust[, censyear := censusyear]
censcmsm<-merge(Aust, tbhazl, by.x = c("censyear"), by.y = c("year"), all.x=TRUE, all.y=FALSE)
...
#Merging it to the Australian census data information
yoatocens <- merge(Aust[,1:7], yoatocens,by=c('id'))


##Adding up all of the hazards!!! i.e. yob+yobtoyoa+yoa+yoatocens
yobtoyoa<-yob[,8:207]+yobtoyoa[,8:207]+yoa[,8:207]+yoatocens[,8:207]
#Merging it to the Australian census data information
yobtoyoa$id <- seq.int(nrow(yobtoyoa))
yobtoyoa <- merge(Aust[,1:7], yobtoyoa,by=c('id'))
rm(yoa)
